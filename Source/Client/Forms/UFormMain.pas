...
uses
  ...
  UFormWait, UFormLogin, UFormBase,UMgrLang;
...
begin
  gMultiLangManager.SectionID := TfMainForm.ClassName;
...
begin
  ...
  nLabel.Caption := MLang(gSysParam.FHintText);
  nLabel.Left := 8;
...
begin
  ...
  gStatusBar := sBar;
  nStr := Format(MLang(sDate), [DateToStr(Now)]);
  StatusBarMsg(nStr, cSBar_Date);

  nStr := Format(MLang(sTime), [TimeToStr(Now)]);
  StatusBarMsg(nStr, cSBar_Time);

  nStr := Format(MLang(sUser), [gSysParam.FUserName]);
...
begin
  ...
  begin
    ...
    Caption := MLang(gSysParam.FMainTitle) + ' V' + nStr;
  end else Caption := MLang(gSysParam.FMainTitle);
  ...
  begin
    ...
    RunSystemObject;
    gMultiLangManager.SectionID := 'TfMainForm';
...
begin
  ...
  {$IFNDEF debug}
  if not QueryDlg(MLang(sCloseQuery),MLang(sHint)) then
  begin
    Action := caNone; Exit;
  end;
  {$ENDIF}
  ...
  ShowWaitForm(Self, MLang('正在退出系统'), True);
  try
    FormSaveConfig;          //窗体配置
...
begin
  sBar.Panels[cSBar_Date].Text := Format(MLang(sDate), [DateToStr(Now)]);
  sBar.Panels[cSBar_Time].Text := Format(MLang(sTime), [TimeToStr(Now)]);
end;
