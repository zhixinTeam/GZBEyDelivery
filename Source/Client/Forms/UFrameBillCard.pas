...
begin
  gMultiLangManager.SectionID := TfFrameBillCard.ClassName;
...
begin
  ...
  if FQueryHas then
  begin
    nStr := 'Select bc.*,L_ID,L_CusID,L_CusName,L_Truck=(case when ' +
            'L_Truck is null then C_TruckNo else L_Truck end) From $BC bc ' +
...
var nBill,nTruck,nLang,nHint: string;
begin
  if BtnAdd.Enabled then
  begin
    nBill := '';
...
var nStr,nSQL,nLang,nHint: string;
begin
  if cxView1.DataController.GetSelectedCount < 1 then
  begin
    nLang := MLang('请选择要删除的磁卡');
    nHint := MLang(sHint);
  ...
  nSQL := MLang('确定要对卡[ %s ]执行删除操作吗?');
  nStr := SQLQuery.FieldByName('C_Card').AsString;

  nLang := MLang(nSQL);

  nSQL := Format(nLang, [nStr]);
  if not QueryDlg(nSQL, MLang(sAsk)) then Exit;
...
begin
  ...
  begin
    nELabel := SQLQuery.FieldByName('C_Type').AsString = sFlag_CardTypeEL;
    nStr := SQLQuery.FieldByName('C_Status').AsString;
    ...
    //已挂失卡可以解挂失
    N11.Enabled := (nStr = sFlag_CardLoss) and (not nELabel);
    //已挂失卡可以补办卡

    N12.Enabled := (nStr <> sFlag_CardInvalid) and (not nELabel);
    //可随时销卡
  end;
...
begin
  ...
  nStr := Format(MLang('确定要对卡[ %s ]执行销卡操作吗?'), [nCard]);
  ...
  begin
    InitFormData(FWhere);
    ShowMsg(MLang('注销操作成功'), MLang(sHint));
  end;
end;
