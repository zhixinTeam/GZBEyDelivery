...
uses
  ...
  UFormWait, USysConst, USysDB,UMgrLang;

function MLang(const nStr: string): string;
begin
  gMultiLangManager.SectionID := TfFramePoundQuery.ClassName;
...
var nIdx: Integer;
    nStr,nID,nP,nLang,nError,nHint: string;
begin
  ...
  nStr := Format(MLang('确定要删除编号为[ %s ]的过磅单吗?'), [nID]);
  nLang :=  MLang(nStr);
  if not QueryDlg(nLang, MLang(sAsk)) then Exit;
  ...
  try
    ...
    nLang :=  MLang('删除失败');
    nError :=  MLang(sError);
    ShowMsg(nLang, nError);
  end;
end;

//Desc: 查看抓拍
procedure TfFramePoundQuery.N4Click(Sender: TObject);
var nStr,nID,nDir,nLang: string;
    nPic: TPicture;
begin
  if cxView1.DataController.GetSelectedCount < 1 then
  begin
    nLang :=  MLang('请选择要查看的记录');
    ShowMsg(nLang, sHint);
    Exit;
  end;
