...
uses
  ...
  UFormInputbox,UMgrLang;

function MLang(const nStr: string): string;
begin
  gMultiLangManager.SectionID := TfFrameQueryDispatch.ClassName;
...
var nDate: TDateTime;
    nStr,nTruck,nStock,nLang,nHint: string;
begin
...
var nStr,nLine,nTmp,nLang,nHint: string;
begin
  ...
  nLine := nStr;
  nLang := MLang('请输入新的装车通道号');
  nHint := MLang(sHint);
  if not ShowInputBox(nLang, nHint, nLine, 15) then Exit;
  ...
  begin
    ...
    begin
      nStr := '通道[ %s ]的水泥品种与待装品种不一致,详情如下:' + #13#10#13#10 +
              ...
              '确定要定道操作吗?';
      nLang := MLang(nStr);
      nStr := Format(nLang, [nLine, Fields[1].AsString, nTmp]);
      if not QueryDlg(nStr, MLang(sAsk)) then Exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    nLang := MLang(nStr);
    nHint := MLang(sAsk);
    if not QueryDlg(nLang, nHint) then Exit;
    ...
    nStr := Format(MLang('清理队列,共[ %d ]张交货单出队.'), [nInt]);
    FDM.WriteSysLog(sFlag_TruckQueue, sFlag_TruckQueue, nLang);
