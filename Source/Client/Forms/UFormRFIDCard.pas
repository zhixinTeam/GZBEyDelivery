...
begin
  gMultiLangManager.SectionID := TfFormRFIDCard.ClassName;
...
begin
  ...
  begin
    ...
    ShowMsg(MLang('请输入有效电子标签'), MLang(sHint));
    Exit;
  end;
...
begin
  ...
  try
    ...
    if FPort < 0 then
    begin
      edtRFIDCard.Properties.ReadOnly := False;
      Result := True;
      Exit;
    end;
  finally
  ...
  if nInt <> 0 then
  begin
    nStr := MLang('电子标签读卡器打开失败,错误码[0x%s]');
    nStr := Format(nStr, [IntToHex(nInt, 2)]);
    //xxxxxx

    WriteLog(nStr);
    ShowMsg(nStr, MLang('提示'));
    Exit;
  end;
