...
uses
  ...
  USysDataDict, USysPopedom, USysDB,UMgrLang;

function MLang(const nStr: string): string;
begin
  gMultiLangManager.SectionID := TfFrameNormal.ClassName;
...
begin
  ...
  ShowWaitForm(ParentForm, MLang('读取数据'));
...
begin
  ...
  try
    ShowMsgOnLastPanelOfStatusBar(MLang('正在读取数据,请稍候...'));
    nBool := True;
...
begin
  ...
  else ShowMsg(MLang('没有可以导出的数据'), MLang(sHint));
end;
...
begin
  ...
  else ShowMsg(MLang('没有可以打印的数据'), MLang(sHint));
end;
...
begin
  ...
  else ShowMsg(MLang('没有可以预览的数据'),MLang(sHint));
end;
