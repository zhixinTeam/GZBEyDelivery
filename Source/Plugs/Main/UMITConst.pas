...
uses
  Windows, SysUtils, Classes, ComCtrls, Forms, IniFiles, USysMAC, UMemDataPool;
...
type
  ...
  PWorkerBusinessLang = ^TWorkerBusinessLang;
  TWorkerBusinessLang = record
    FProcessID: Int64;                               //线程标识
    FLangID: string;                                 //语言标识
  end;
...
var
  ...
  gRegistryMultiLangData: Integer = 0;               //多语言注册标记
...
//在状态栏显示信息
function GetMultiLangID: string;
//获取线程的多语言标识
...
//------------------------------------------------------------------------------
function EnumMemData(const nData: Pointer; const nResult: TList): Boolean;
var nLang: PWorkerBusinessLang;
begin
  Result := True;
  if PWorkerBusinessLang(nData).FProcessID <> GetCurrentThreadId then Exit;

  nLang := gMemDataManager.LockData('LangID');
  nResult.Add(nLang);

  nLang.FProcessID := 0;
  nLang.FLangID := PWorkerBusinessLang(nData).FLangID;
  Result := False;
end;
...
begin
  nList := TList.Create;
  try
    gMemDataManager.EnumData(0, 'LangID', EnumMemData, nList);
    if nList.Count > 0 then
         Result := PWorkerBusinessLang(nList[0]).FLangID
    else Result := '';
  finally
